(function()
{
	var Slider =
	{
		...
		render: function()
		{
			...
			{
				this.$('.unit_title', this.view.self).text(this.unitVariation.title.replace('Deals','Results'));
				this.$('.vertical_text', this.view.self).text('Results');
                this.$('.side_slider_header', this.view.self).addClass('qwerfv');
                this.$('.header_tongue', this.view.self).addClass('qwerfv');
                this.$('.offers_count', this.view.self).addClass('qwerfv');
			}
		},
